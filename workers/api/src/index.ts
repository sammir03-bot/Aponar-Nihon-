...
7. A specific requested length or example count overrides the general teaching templates: when asked for one short example, give exactly one and omit extra exercises. In every translation preserve the Japanese subject, tense, negation and speech act; do not turn an invitation (〜ませんか) into a negative command. Bangla translations must be grammatical Bangla with matching subject and verb (আমি ... করেছি, শিশু ... করেছে). Do not invent Bengali-script pronunciations of Japanese words. For Bangla, read each translation as a standalone sentence and simplify it if it sounds unnatural.
   Translation calibration: 道（みち）で財布（さいふ）を拾（ひろ）いました。 = রাস্তায় একটি মানিব্যাগ কুড়িয়ে পেয়েছি। 子（こ）どもがおもちゃを拾（ひろ）いました。 = শিশুটি খেলনাটি কুড়িয়ে নিয়েছে। 一緒（いっしょ）に勉強（べんきょう）しませんか。 = একসঙ্গে পড়াশোনা করবেন? Never add an unsupported subject change or negative instruction.
7a. Keep simple answers compact. If the learner asks for detail or an easy step-by-step explanation in any supported language, or asks about confusing grammar, teach thoroughly and step-by-step.
...
8. Write for a native speaker with plain, familiar wording. Use established local interface terms; preserve employer/product names such as TownWork and Kura Sushi. Natural loanwords in Vietnamese and Filipino are allowed. Avoid literal Bangla word order.
...
function translationHasLegacyProse(value: string, targetLanguage: TutorLanguage, source = ""): boolean {
  let residue = translationResidue(value);
  // A foreign employer/product name is not untranslated interface prose.
  const names = source.match(/\b(?:TownWork|Baitoru|dipID|dip|Kura Sushi|FamilyMart|Lawson|7-Eleven|Seven-Eleven|McDonald['’]?s|Sushiro|Saizeriya|Sukiya|Yoshinoya|Matsuya|MOS Burger|Gusto|KFC|AEON|MyNavi|YOLO Japan|Hello Work|HelloWork|Indeed|LinkedIn|GaijinPot|Daijob|Minna no Nihongo|Matome|NHK)\b/gi) || [];
  ...
  async function worker(): Promise<void> {
    while (cursor < input.items.length) {
      ...
      if (sourceLanguage === input.targetLanguage) {
        if (translationHasLegacyProse(item.text, input.targetLanguage, item.text)) {
