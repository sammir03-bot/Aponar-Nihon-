...
export default function ProfileScreen() {
  ...
  async function submit() {
    if (!supabase) { setMessage('Supabase publishable key configure করা হয়নি।'); return; }
    setMessage('');
    if (signup) {
      const { error } = await supabase.auth.signUp({ email: email.trim(), password, options: { emailRedirectTo: 'aponarnihon://auth/callback' } });
  ...
  if (!supabase) return <Screen><SectionTitle title="Profile" /><Text style={styles.message}>Mobile `.env`-এ Supabase publishable key যোগ করলে login/profile চালু হবে। Secret/service-role key কখনো app-এ রাখা হবে না।</Text></Screen>;
  if (session) return <Screen><SectionTitle title="আপনার Profile" subtitle="Website ও app একই Supabase account ব্যবহার করে।" /><View style={styles.card}><Text style={styles.email}>{session.user.email}</Text><Text style={styles.meta}>User ID: {session.user.id}</Text><Text style={styles.meta}>Progress records: {progressCount ?? '…'}</Text></View><PrimaryButton label="Daily study reminder চালু করুন" onPress={reminder} /><SecondaryButton label="Logout" onPress={async () => { await supabase.auth.signOut(); setMessage('Logout হয়েছে।'); }} />{message ? <Text style={styles.message}>{message}</Text> : null}</Screen>;
